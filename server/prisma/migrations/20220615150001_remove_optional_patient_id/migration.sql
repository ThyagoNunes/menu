/*
  ...
  - Made the column `patientId` on table `beds` required. This step will fail if there are existing NULL values in that column.
...
CREATE TABLE "new_beds" (
    ...
    CONSTRAINT "beds_patientId_fkey" FOREIGN KEY ("patientId") REFERENCES "patients" ("id") ON DELETE RESTRICT ON UPDATE CASCADE
);
